...
public class CalculatorTest {
    ...
    @Test
    public void positiveIntegerPow() {
        double resultMul = mCalculator.pow(2d, 3d);
        assertThat(resultMul, is(equalTo(8d)));
    }

    @Test
    public void firstNegativePow() {
        double resultMul = mCalculator.pow(-2d, 3d);
        assertThat(resultMul, is(equalTo(-8d)));
    }

    @Test
    public void secondNegativePow() {
        double resultMul = mCalculator.pow(2d, -3d);
        assertThat(resultMul, is(equalTo(0.125d)));
    }

    @Test
    public void firstZeroPow() {
        double resultMul = mCalculator.pow(0d, 3d);
        assertThat(resultMul, is(equalTo(0d)));
    }

    @Test
    public void secondZeroPow() {
        double resultMul = mCalculator.pow(3d, 0d);
        assertThat(resultMul, is(equalTo(1d)));
    }

    @Test
    public void firstZeroSecondNegativeOnePow() {
        double resultMul = mCalculator.pow(0d, -1d);
        assertThat(resultMul, is(equalTo(Double.POSITIVE_INFINITY)));
    }

    @Test
    public void firstNegativeZeroSecondNegativePow() {
        double resultMul = mCalculator.pow(-0d, -1d);
